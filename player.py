...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.available_moves()) #if there is a full board, just pick a random square
        else:
            # now we need a square based off the minimax algorithm
            square = self.minimax(game, self.letter)['position']
        return square
    
    def minimax(self, state, player):
        max_player = self.letter #this is you, the player
        other_player = 'O' if player == 'X' else 'X' # the other player

        #first, we want to check if the previous move is a winner
        #in recursion we need a base case, at the very end of things, where are we at?
        if state.current_winner == other_player:
            # we should return the position and the score because we need to keep track of score
            #for minimax to work
        ...
        for possible_move in state.available_moves():
            ...
            else: # but also minimize the other player
